...
def validar_componente(componente, indice):
    ...
    errores_campo = validar_presencia_campos(
        componente, CAMPOS_REQUERIDOS_COMPONENTE, f"Component[{indice}]"
    )
    ...
    if (
        "name" in componente
        and isinstance(componente["name"], str)
        and not componente["name"].strip()
    ):
    ...
    if (
        "version" in componente
        and isinstance(componente["version"], str)
        and not componente["version"].strip()
    ):
