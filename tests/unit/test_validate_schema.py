...
@pytest.mark.parametrize(
    "datos,campos,contexto,errores_esperados",
    [
        ...
        (
            ...
            ["name", "version"],
            "Test",
            ["Test: campo requerido 'version' no encontrado"],
        ),
        (
            {},
            ["name", "version"],
            "Test",
            [
                "Test: campo requerido 'name' no encontrado",
                "Test: campo requerido 'version' no encontrado",
            ],
        ),
    ],
)
def test_validar_presencia_campos_parametrizado(
    datos, campos, contexto, errores_esperados
):
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"name": "pkg", "version": "", "type": "lib"},
            "'version' no puede estar vacio",
        ),
    ],
)
...
# campos faltantes
@pytest.mark.parametrize(
    "metadata,error_substring",
    [
        (
            {"component": {"type": "app", "name": "test", "version": "1.0"}},
...
def test_validar_esquema_sbom_casos_limite_parametrizado(
    temp_sbom_file, request, fixture_name, error_substring
):
...
def test_validar_esquema_sbom_components_no_array(
    temp_sbom_file, sbom_components_no_array
):
...
@patch(
    ...
    read_data='{"bomFormat": "CycloneDX", "specVersion": "1.4", "version": 1, \
