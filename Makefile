...
install: install-env install-hooks install-gitleaks install-infra-tools ## Instala dependencias y configura el entorno
...
tf-validate: ## Ejecuta terraform fmt -check y terraform validate
...
clean: ## Borra los archivos generados
	@sudo rm -rf .venv .pytest_cache/ .ruff_cache/
	@sudo rm -rf infra/terraform/.infra infra/terraform/.terraform infra/terraform/terraform.* infra/terraform/.terraform.*
	@sudo rm sbom.json dashboard.html
	@sudo rm -rf .pytest_cache/ .ruff_cache/